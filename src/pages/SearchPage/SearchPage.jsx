      ...
      <div className="container-md p-0 pb-5 mx-3" style={{ maxWidth: '390px', backgroundColor: 'white', margin: 'auto', height: '100%', overflow: 'auto'}}>
        ...
        <div>
          <div className="row container-lg gap-2 d-flex justify-content-center align-items-center mx-auto mt-1" id="list-products">
            {notFound ? (
              <span className='text-center fw-bold' style={{ color: '#B31312' }}>Hasil tidak ditemukan</span>
            ) : (
              filteredItems.map((item) => (
                <div key={item._id} className="card col-md-6 p-0" style={{ width: '48%', height: '280px' }}>
                  <Link to={`/homepage/search/produk/detail/${item._id}`} style={{ textDecoration: 'none', color: 'black' }}>
                    <img src={item.image || 'https://i.imgur.com/2a0RWOy.jpg'} className="card-img-top" style={{objectFit: 'cover'}} width="120px"
                      onError={(e) => {
                    ...
                    <div className="card-body">
                      <h5 style={{ fontSize: "14px" }} className="card-title fw-semibold">{item.nama_produk}</h5>
                      <p className="card-text fw-bold" style={{color: '#2B2A4C', fontSize: "14px"}}><span>Rp </span>{item.harga}</p>
                      <div className="rating ms-1" style={{ position: 'absolute', bottom: '10px', left: '10px', right: '10px' }}>
                        <div className="d-flex align-items-center">
                          ...
                          <span className='fs-6 ms-1 fw-bold' style={{ color:"#ea906c" }}>{item.rating || <span>0,0</span>}</span>
                        </div>
                      </div>
                    </div>
                  </Link>
